...
class SeverityClassifier:
    ...
    async def classify_severity(
        ...
        try:
            ...
            if self.ai_engine:
                logger.debug(
                    f"Using AIEngine for severity classification with model: {self.ai_engine.llm_model}"
                )
                ...
                response = await self.ai_engine.llm.ainvoke(
                    [
                        {
                            "role": "system",
                            "content": "You are an incident severity classifier. "
                ...
                severity = self._extract_severity_from_response(content)
                logger.info(f"AIEngine classified incident severity as {severity}")
                return severity

            # Fall back to standard OpenAI client if no AIEngine
            logger.debug(
                f"Using standard OpenAI client for severity classification with model: {self.model}"
            )
            ...
            # Extract and validate severity from response
            severity = self._extract_severity_from_response(
    ...
    def _build_classification_prompt(
        ...
        if events_data and len(events_data) > 0:
            ...
            for event in events_data[
                :3
            ]:  # Limit to first 3 events to keep prompt size reasonable
